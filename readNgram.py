...
wordFile_mf = open('words_1gm_usersMale.txt', 'w')
wordFile_ff = open('words_1gm_usersFemale.txt', 'w')
wordFile_uf = open('words_1gm_usersUnkwown.txt', 'w')

sortedFreqListFem = [[]]
sortedFreqListMale = [[]]
sortedFreqListUnknown = [[]]
...
with open('en.1grams') as f:
    for line in f:
        ...
        if femaleFrequency > 0 or maleFrequency > 0 or unknownFrequency>0:
            sortedFreqListFem.append([femaleFrequency, word[0]])
            sortedFreqListMale.append([maleFrequency, word[0]])
            sortedFreqListUnknown.append([unknownFrequency, word[0]])
...
for item in sortedFreqListUnknown:
    wordFile_uf.write(str(item[0]) + '\t' + str(item[1]) + '\n')
